...
int ipme_append_unless(ix, ix_mask, notip, notip_mask)
  struct ip_mx *ix, *ix_mask;
  struct ipalloc *notip, *notip_mask;
{
  int i;
  for (i = 0;i < notip->len;++i)
    if (byte_equal(&notip->ix[i].ip,4,&ix->ip) &&
        byte_equal(&notip_mask->ix[i].ip,4,&ix_mask->ip))
      return 1;
